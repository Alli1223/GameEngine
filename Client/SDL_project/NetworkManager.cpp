...
void NetworkManager::NetworkUpdate(World& world, Player& player)
{
	// Interval Timer
	...
	try
	{
		...
		for (auto& element : playerData)
		{
			...
			if (DoesPlayerExist(otherPlayerNames, name))
			{
				...
				allPlayers[val]->PlayerClothes.hat = (Player::Clothing::HeadWear)headWear;
